...
def test_lock_manager():
    ...
    assert not transaction2.create_operation(OperationType.READ, "resource1")  # Deveria falhar

    # Liberar bloqueio
    transaction1.commit_transaction()
    assert transaction2.create_operation(OperationType.READ, "resource1")  # Deveria ter sucesso agora
